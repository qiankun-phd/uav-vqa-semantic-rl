#!/usr/bin/env bash
# Task #36 v7b: WIRING FIX re-run of the v7 matrix.  v7 shipped mission_aligned
# (#36) into multi_uav_env._reward() only, but the two-timescale PPO learner
# optimises the CONTROLLER reward (_semantic_controller_reward - dual penalty),
# which under semantic_reward_mode=semantic_utility RECOMPUTES the reward from
# info and DISCARDS raw_reward -- so the env-layer mission_aligned signal never
# reached the gradient (v7 proposed/no_lagrangian were bit-identical to legacy
# v6 while only raw_return moved in the log).  v7b replicates mission_aligned in
# the controller reward + the shaped-reward dual conflict penalty
# (src/vqa_semcom/rl/v19_ppo.py::_mission_aligned_reward_info), gated on
# --reward-success-semantics.  No flag/gate change from v7: the SAME CLI now
# feeds BOTH layers.  Output outputs/rl/eps_recal_v7b, tmux epsv7b.
#
# --- original v7 header retained below ---
...
# Output under outputs/rl/eps_recal_v7b.  tmux epsv7.
set -u
...
TPE="${TPE:-20}"
SEEDS="${SEEDS:-0 1 2}"
OUT="${OUT:-outputs/rl/eps_recal_v7b}"
...
run_one() {
  ...
  if [ -f "$dir/run_config.json" ]; then
    echo "[recal7b] SKIP  arm=$arm seed=$seed${DIR_SUFFIX:-} (done)"
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$arm${DIR_SUFFIX:-}" "$seed" "skip" "-" "-" "$dir" >> "$STATUS"
    return 0
  fi
  local start; start="$(date +%Y-%m-%dT%H:%M:%S)"
  echo "[recal7b] START arm=$arm seed=$seed${DIR_SUFFIX:-} $(date)"
  ...
  echo "[recal7b] END   arm=$arm seed=$seed${DIR_SUFFIX:-} exit=$ec $(date)"
  return "$ec"
}
...
run_nom_eval() {
  ...
  if [ ! -f "$model" ]; then
    echo "[recal7b] MISS  arm=$arm seed=$seed nom-eval (no ckpt $model)"
...
echo "[recal7b] ==== epsilon recalibration v7b (wiring fix) start $(date) | cal=$CAL ccc=$CCC cacheq=$CACHEQ esc=$ESC qb=$QB rss=$RSS refbw=$REFBW lutg=$LUTG delta_peak=$DELTA_PEAK delta_nom=$DELTA_NOM train_ep=$TRAIN_EP seeds=$SEEDS ===="
...
echo "[recal7b] ==== v7b run done $(date) ===="
cat "$STATUS"
