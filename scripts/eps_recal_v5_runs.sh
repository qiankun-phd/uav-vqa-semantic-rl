#!/usr/bin/env bash
# Task #28: epsilon recalibration iteration 5 -- attainable epsilon + escalation.
#
...
#
# delta_esc (escalation budget) is condition-adaptive: peak (utm_conflict) ~0.90,
# nominal ~0.50 (see scripts/calibrate_epsilon_v5.py / EPSILON_RECAL_V5.md).
#
...
TPE="${TPE:-20}"
SEEDS="${SEEDS:-0 1 2}"
OUT="${OUT:-outputs/rl/eps_recal_v5}"
...
ESC="spec_attainable"
QB="lut_v5"
DELTA_PEAK="${DELTA_PEAK:-0.90}"
DELTA_NOM="${DELTA_NOM:-0.50}"
mkdir -p "$OUT"
...
run_one() {
  ...
  if [ -f "$dir/run_config.json" ]; then
    echo "[recal5] SKIP  arm=$arm seed=$seed${DIR_SUFFIX:-} (done)"
    printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$arm${DIR_SUFFIX:-}" "$seed" "skip" "-" "-" "$dir" >> "$STATUS"
    return 0
  fi
  local start; start="$(date +%Y-%m-%dT%H:%M:%S)"
  echo "[recal5] START arm=$arm seed=$seed${DIR_SUFFIX:-} $(date)"
  ...
  echo "[recal5] END   arm=$arm seed=$seed${DIR_SUFFIX:-} exit=$ec $(date)"
  return "$ec"
}
...
run_nom_eval() {
  ...
  if [ ! -f "$model" ]; then
    echo "[recal5] MISS  arm=$arm seed=$seed nom-eval (no ckpt $model)"
...
echo "[recal5] ==== epsilon recalibration v5 start $(date) | cal=$CAL ccc=$CCC cacheq=$CACHEQ esc=$ESC qb=$QB delta_peak=$DELTA_PEAK delta_nom=$DELTA_NOM train_ep=$TRAIN_EP seeds=$SEEDS ===="
...
for seed in $SEEDS; do
  DIR_SUFFIX="_nomtrain" run_one proposed "$seed" \
    ...
    --conflict-cost-weight 0.0 --queue-risk-weight 0.0 --queue-utm-weight 0.0 \
    --lambda-lr-conflict 0.2 --lambda-max-conflict 8.0 --conflict-cost-limit 0.08 \
    "${V5_GATES[@]:0:8}" --quality-backend "$QB" \
    --scenario "$NOM_SCENARIO" --escalation-cost-limit "$DELTA_NOM" \
    --train-episodes "$TRAIN_EP" --episodes "$EVAL_EP" --tasks-per-episode "$TPE"
done

echo "[recal5] ==== v5 run done $(date) ===="
cat "$STATUS"
