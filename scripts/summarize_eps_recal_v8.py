#!/usr/bin/env python
"""Task #37 v8: lambda_QC unpin (per-channel quality cap + dual warm-up) + judgment.

v8 re-runs the v7b matrix with two levers aimed at the quality-critical dual pin
(v7b lambda_QC ratcheted to ~18.4 under quality_cost_critical ~0.40 >> 0.02):
lever 1 caps the quality channels at lambda_max_quality=8 (conflict-channel
precedent), lever 2 freezes all duals for the first 150 episodes (BC/service-
prior shaping window) then resumes ascent.  Reads outputs/rl/eps_recal_v8.  Same
13 v6/v7 criteria; the ones the levers should move are 3a cacheR, 3b rejectR,
3c mission, 3d acc, 4 lambda_QC (the pin), 5b nominal task, and 5a nominal
mission back to PASS.  Criterion 4's "non-pinned band" upper edge is now
0.9 * lambda_max_quality (7.2), since 8 is the new ceiling.

Original v7b docstring:
...
ROOT = BASE / "eps_recal_v8"
CALIB = BASE / "eps_v7_calib_corrected.json"
LAMBDA_MAX = 20.0
# v8 lever 1: the quality channels cap at 8 (per-channel), so criterion-4's
# non-pinned band for lambda_QC uses this ceiling, not the global lambda_max.
LAMBDA_MAX_QUALITY = 8.0
...
print("=" * 140)
print("LAMBDA / ESCALATION TRAJECTORY (v8 peak training, terminal-window means; lamQC cap=8, warmup=150)")
...
# on >= 2/3 seeds.  "non-pinned" = terminal in (0.1, 0.9*ceiling) AND not stuck
# at the max; OR the quality-critical cost sits in the target band.  v8: the
# quality-critical channel ceiling is lambda_max_quality (8), so a lambda_QC that
# saturates at ~8 is still a pin FAIL (the pinned flag catches the no-fall-back
# shape regardless of the numeric ceiling); the deadline channel keeps the global
# lambda_max ceiling.
active = []
for seed in (0, 1, 2):
    sh = lam(ROOT / f"proposed_{seed}")
    if sh:
        band_q = (0.1 < sh["last"] < 0.9 * LAMBDA_MAX_QUALITY) and not sh["pinned"]
