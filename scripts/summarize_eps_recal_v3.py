#!/usr/bin/env python
"""Task #28 iteration 3: 4-generation comparison legacy / v1 / v2 / v3 + v3 criteria.
...
# "clearly off the 0.10 magnitude" threshold for peak proposed semSucc.
SEMSUCC_OFF_TENTH = 0.25
...
for cond in ("peak", "nominal"):
    print("=" * 104)
    print(f"4-GEN COMPARISON -- {cond} condition")
    print("=" * 104)
...
# lambda trajectory shape (v3 peak training)
print("=" * 104)
print("LAMBDA_QUALITY_CRITICAL TRAJECTORY (v3 peak training)")
print("=" * 104)
...
for arm in ["proposed", "no_lagrangian", "fixed_penalty", "e4lut"]:
    for seed in (0, 1, 2):
        sh = lam(ROOT / "eps_recal_v3" / f"{arm}_{seed}")
        if sh is None:
            continue
...
print("=" * 104)
checks = []
o = m("v3", "bl_oracle_best_feasible_evidence", "peak", "semantic_success_rate")
checks.append(("PEAK oracle semSucc >= 0.85 (structural rule, cache non-compliant)", o, o >= 0.85))
pc = m("v3", "proposed", "peak", "semantic_path_cache_ratio")
pa = m("v3", "proposed", "peak", "average_accuracy")
ps = m("v3", "proposed", "peak", "semantic_success_rate")
checks.append(("PEAK proposed cache ratio < 0.40", pc, pc < 0.40))
checks.append(("PEAK proposed average_accuracy >= 0.45", pa, pa >= 0.45))
checks.append((f"PEAK proposed semSucc off the 0.10 band (>= {SEMSUCC_OFF_TENTH})", ps, ps >= SEMSUCC_OFF_TENTH))
turned = []
for seed in (0, 1, 2):
    sh = lam(ROOT / "eps_recal_v3" / f"proposed_{seed}")
    if sh:
        turned.append(not sh["pinned"])
checks.append(("PEAK proposed lambda_critical not pinned (>=2/3 seeds)", sum(turned), sum(turned) >= 2))
ns = m("v3", "proposed", "nominal", "semantic_success_rate")
nt = m("v3", "proposed", "nominal", "task_success_rate")
checks.append(("NOMINAL proposed semSucc >= 0.89 (v1 level 0.897)", ns, ns >= 0.89))
