...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diagnose semantic-path and mobility feasibility bottlenecks.")
    parser.add_argument("--config", default="configs/v1_9_snr_lut.yaml")
    parser.add_argument("--output-dir", default="outputs/env/semantic_path_bottleneck_diagnosis_20260624")
...
def _report(summary: list[dict[str, Any]], mobility: list[dict[str, Any]]) -> str:
    ...
    for row in summary:
        lines.append(
            ...
            f"{float(row['utm_feasible_ratio']):.3f} | {row['bottleneck_distribution']} | "
