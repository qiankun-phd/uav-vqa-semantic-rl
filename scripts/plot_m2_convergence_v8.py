...
for name, col, lab in (("peak", COL_PEAK, "peak-trained"), ("nomtrain", COL_NOM, "nominal-trained")):
    ...
    band(ax, ep, smooth(mean, 15), smooth(std, 15), col, lab, do_smooth=False)
...
ax.annotate(f"limit $d_{{QC}}={QC_LIMIT}$", xy=(0.35, QC_LIMIT), xycoords=("axes fraction", "data"),
            xytext=(0, 2.5), textcoords="offset points", fontsize=6.5)
ax.set_xlabel("training episode")
...
ax.annotate("cap $\\lambda^{max}_{Q}=8$", xy=(0.99, LAMBDA_CAP_Q), xycoords=("axes fraction", "data"),
            xytext=(0, -8), textcoords="offset points", ha="right", fontsize=6.5, color=COL_PEAK)
ax.axvspan(0, DUAL_WARMUP, color="gray", alpha=0.12, lw=0)
ax.annotate("dual\nwarm-up", xy=(DUAL_WARMUP / 2, 0.985), xycoords=("data", "axes fraction"),
