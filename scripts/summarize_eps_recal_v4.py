#!/usr/bin/env python
"""Task #28 iteration 4: 5-generation comparison legacy/v1/v2/v3/v4 + v4 criteria.

legacy = outputs/rl/matrix_v1      (0.82/0.65, cache allowed)
v1     = outputs/rl/eps_recal_v1   (attainability_v1, 0.615/0.166, cache allowed)
v2     = outputs/rl/eps_recal_v2   (attainability_v2, 0.633/0.297, cache allowed)
v3     = outputs/rl/eps_recal_v3   (attainability_v3, 0.504/0.166, cache FORBIDDEN)
v4     = outputs/rl/eps_recal_v4   (attainability_v4, 0.355/0.166, cache FORBIDDEN, tx-only anchor)
"""
...
GENS = [("legacy", ROOT / "matrix_v1"),
        ...
        ("v3", ROOT / "eps_recal_v3"),
        ("v4", ROOT / "eps_recal_v4")]
...
METRICS = ["average_epsilon_k", "semantic_success_rate", "quality_violation_rate",
           ...
           "service_level_0_ratio", "service_level_1_ratio", "service_level_2_ratio",
           "deadline_violation_rate", "average_payload_kb", "reject_ratio"]
...
for cond in ("peak", "nominal"):
    print("=" * 118)
    print(f"5-GEN COMPARISON -- {cond} condition")
    print("=" * 118)
    hdr = (f"{'arm':32} {'gen':6} {'eps':>6} {'semSucc':>8} {'qViol':>7} {'taskSucc':>9} "
           f"{'acc':>6} {'cacheR':>7} {'rejectR':>8} {'ddlVio':>7}")
    print(hdr)
    print("-" * len(hdr))
    for arm in ARMS:
        for tag, _ in GENS:
            ...
            print(f"{arm:32} {tag:6} "
                  ...
                  f"{agg(rows,'reject_ratio')[0]:8.3f} "
                  f"{agg(rows,'deadline_violation_rate')[0]:7.3f}")
        print()

# lambda trajectory shape (v4 peak training)
print("=" * 118)
print("LAMBDA_QUALITY_CRITICAL TRAJECTORY (v4 peak training)")
print("=" * 118)
...
for arm in ["proposed", "no_lagrangian", "fixed_penalty", "e4lut"]:
    for seed in (0, 1, 2):
        sh = lam(ROOT / "eps_recal_v4" / f"{arm}_{seed}")
        if sh is None:
            continue
...
print("\n" + "=" * 118)
print("V4 SUCCESS CRITERIA")
print("=" * 118)
checks = []
o = m("v4", "bl_oracle_best_feasible_evidence", "peak", "semantic_success_rate")
oa = m("v4", "bl_oracle_best_feasible_evidence", "peak", "average_accuracy")
checks.append(("PEAK oracle semSucc >= 0.85 (tx-only anchor construction guarantee)", o, o >= 0.85))
pc = m("v4", "proposed", "peak", "semantic_path_cache_ratio")
pr = m("v4", "proposed", "peak", "reject_ratio")
pa = m("v4", "proposed", "peak", "average_accuracy")
ps = m("v4", "proposed", "peak", "semantic_success_rate")
checks.append(("PEAK proposed cache ratio < 0.40", pc, pc < 0.40))
checks.append(("PEAK proposed reject ratio < 0.20", pr, pr < 0.20))
checks.append((f"PEAK proposed semSucc >= 0.5*oracle ({0.5*o:.3f})", ps, ps >= 0.5 * o))
checks.append((f"PEAK proposed acc >= 0.7*oracle_acc ({0.7*oa:.3f})", pa, pa >= 0.7 * oa))
turned = []
for seed in (0, 1, 2):
    sh = lam(ROOT / "eps_recal_v4" / f"proposed_{seed}")
    if sh:
        turned.append(not sh["pinned"])
checks.append(("PEAK proposed lambda_critical not pinned (>=2/3 seeds)", sum(turned), sum(turned) >= 2))
ns = m("v4", "proposed", "nominal", "semantic_success_rate")
nt = m("v4", "proposed", "nominal", "task_success_rate")
checks.append(("NOMINAL proposed semSucc >= 0.89", ns, ns >= 0.89))
