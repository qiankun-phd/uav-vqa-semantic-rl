...
All research lines are now consolidated on `main` (2026-07: branches
`feat/bubbles-scenario`, `codex/lut-semantic-utility-upgrade`, and
`codex/semantic-path-cache-defer` were merged and deleted; their final tips are
archived as tags `archive/bubbles-final`, `archive/lut-final`,
`archive/cache-defer-final`, with the pre-merge main at
`archive/main-pre-merge`).

The repository hosts two complementary experiment lines:

1. **RL resource-allocation line (v19 + BUBBLES).** Two-timescale constrained
   PPO over semantic paths, mobility, and edge resources, including the
   BUBBLES separation-scenario profile and the full experiment matrix
   (`configs/v1_9_bubbles.yaml`, `scripts/matrix_v1_*.sh`,
   `scripts/ab_bubbles_*`, `scripts/report_matrix_v1.py`;
   tests in `tests/test_v19_correctness.py`,
   `tests/test_v1_9_rl_resource_alloc.py`, `tests/test_bubbles_separation.py`).
2. **Semantic-communication comparison line (LUT / semantic utility).**
   Detector/VLM evaluation, channel degradation, and baseline-comparison
   pipeline (`src/vqa_semcom/{vlm,detector,degradation,evidence,quality,tasks}`;
   `scripts/run_v1_detector_eval.py`, `scripts/build_comparison_v2.py`,
   `scripts/analyze_*.py`, `scripts/chain_*.sh`; configs `configs/v2_0_*`,
   `configs/v25*/v26*`, `configs/dv*`).
...
The current research line uses the VQA-grounded V1.9 semantic utility layer and
